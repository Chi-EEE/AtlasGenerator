...
namespace sc
{
	namespace AtlasGenerator
	{
		...
		public:
			...
			{
			public:
				Transformation(double rotation = 0.0, Point<int32_t> translation = Point<int32_t>(0, 0)) : rotation(rotation), translation(translation)
				{
				}
		...
		public:
			Item(cv::Mat& image, Type type = Type::Sprite) : m_image(image), m_type(type)
			{
			}
			Item(std::filesystem::path path, Type type = Type::Sprite) : m_type(type)
			{
				m_image = cv::imread(path.string(), cv::IMREAD_UNCHANGED);
			}
			Item(cv::Scalar color, Type type = Type::Sprite) : m_type(type)
			{
		...
		public:
			...
			uint16_t width() const { return (uint16_t)m_image.cols; };
			uint16_t height() const { return (uint16_t)m_image.rows; };
